...
from engine_batch import run_fifo_engine, build_value_by_site_status, build_detailed_status_matrix
...
mode = st.radio("اختار نوع التحليل", [
    "التحليل السريع (راكد + تنبؤ نفاذ بسيط)",
    "تحليل الدفعات FIFO المتقدم (Batch Aging + التحويلات)",
], horizontal=False)

if mode.startswith("تحليل الدفعات"):
    st.caption("محتاج الأعمدة الخام من نظامك مباشرة: Physical date, Item number, Variant number, Site, Warehouse, Quantity, Reference, Number, Physical cost amount, Description, Unit, Dimension 1-5")
    up = st.file_uploader("ارفع ملف الإكسيل الخام", type=["xlsx", "xlsb", "xls"], key="batch_upload")
    today = pd.Timestamp(st.date_input("تاريخ التحليل", value=pd.Timestamp.today(), key="batch_date"))

    if up:
        engine = "pyxlsb" if up.name.lower().endswith(".xlsb") else None
        raw = pd.read_excel(up, engine=engine) if engine else pd.read_excel(up)
        required = ["Physical date", "Item number", "Variant number", "Site", "Warehouse",
                    "Quantity", "Reference", "Number", "Physical cost amount"]
        missing = [c for c in required if c not in raw.columns]
        if missing:
            st.error(f"الملف ناقصه الأعمدة دي: {missing}")
            st.stop()
        for opt_col in ["Description", "Unit", "Dimension 1", "Dimension 2", "Dimension 3", "Dimension 4", "Dimension 5"]:
            if opt_col not in raw.columns:
                raw[opt_col] = ""

        if st.button("🔍 شغّل تحليل الدفعات"):
            batch_report, pending_issues = run_fifo_engine(raw, today)
            real_issues = {k: v for k, v in pending_issues.items() if v > 0.01}

            st.subheader("⚠️ تقرير مشاكل البيانات")
            if real_issues:
                st.error(f"لقينا {len(real_issues)} حالة صرف بدون رصيد كافٍ وقت التسجيل (احتمال ترتيب تاريخي غلط أو استلام ناقص)")
                issues_df = pd.DataFrame(
                    [{"الصنف/الفاريانت/الموقع": k, "الكمية غير المغطاة": round(v, 2)} for k, v in real_issues.items()]
                )
                st.dataframe(issues_df, use_container_width=True)
            else:
                st.success("مفيش مشاكل ترتيب تاريخي أو أرصدة سالبة في البيانات دي ✅")

            st.subheader("1️⃣ تقرير الدفعات التفصيلي (Batch Aging Report)")
            st.dataframe(batch_report, use_container_width=True)

            st.subheader("2️⃣ القيمة حسب المشروع والحالة")
            value_by_site = build_value_by_site_status(batch_report)
            st.dataframe(value_by_site, use_container_width=True)

            st.subheader("3️⃣ التقرير التفصيلي بالحالات (Site / Item / Variant)")
            detailed_matrix = build_detailed_status_matrix(batch_report)
            st.dataframe(detailed_matrix, use_container_width=True)

            sheets = {
                "batch_report": batch_report,
                "value_by_site_status": value_by_site,
                "detailed_status_matrix": detailed_matrix,
            }
            if real_issues:
                sheets["data_issues"] = issues_df
            excel_bytes = style_and_export(sheets)
            st.download_button("⬇️ تحميل كل التقارير Excel", data=excel_bytes,
                                file_name="تقرير_الدفعات_الكامل.xlsx",
                                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    st.stop()

st.caption("ارفع شيت الإكسيل بتاع حركة المخازن، والنظام يطلعلك الراكد والتنبؤ بالنفاذ تلقائيًا")

uploaded = st.file_uploader("ارفع ملف الإكسيل", type=["xlsx", "xlsb", "xls"])
